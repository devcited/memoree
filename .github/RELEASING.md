...
6. Create and push the matching annotated tag. The generated Release workflow uploads every archive, checksum, source bundle, and attestation to the draft, then publishes it only after all uploads succeed.
...
8. Deploy the versioned site image through GitOps, then smoke-test `memoree.dev`, the release manifest, redirects, a clean install, and an upgrade of the immutable v0.2.0 fixture with both running and stopped daemon states.
