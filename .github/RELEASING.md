...
1. Update the Cargo version, changelog, and website release metadata in their respective commits. The Kubernetes image tag is not edited by hand; Flux image automation writes it.
...
8. Flux (`memoree-site-main` policy, `memoree-main-images` automation) picks the new tag up from Harbor within a minute and commits the pinned `tag@digest` to the GitOps repo, which rolls `memoree.dev`. Then smoke-test `memoree.dev`, the discovery pointer, signed release manifest, redirects, a clean install, an isolated confirmation-based automatic update, and an upgrade of the immutable v0.2.0 fixture with both running and stopped daemon states.
