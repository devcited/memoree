...
Schema migration is a local control-plane operation rather than a daemon protocol operation. Opening a schema 1–3 store under 0.3 first serializes on a filesystem lock, preflights backup space, publishes a verified old-schema SQLite/CAS snapshot, and only then commits schema 4. The recovery snapshot path is exposed by `upgrade apply`; ordinary daemon protocol calls see only the resulting current schema.
