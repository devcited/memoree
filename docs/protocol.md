...
`memory.recall` is the normal “does memory have something about this?” read. It runs the existing FTS5 path independently for current claims and artifact references, so the two types never compete for the same presentation slots and no separate index or model ranker can drift from `search`. Its bounded input is:
...
`memoree remember` is a caller-side composition over `artifact.put` and `claim.assert`, not a protocol operation. It freezes ambient context, optionally asks an isolated Luna process for a strict claim proposal with one to four exact quotes per claim, verifies every quote and computes its byte span in Rust, emits deterministic source/lifecycle/relation quality findings, previews by default, and submits normal mutations only with `--apply`. The findings do not certify truth or authorize cleanup. Consequently `remember` is intentionally absent from daemon capability and schema operation lists.
...
`doctor` returns a typed health result including `running`, `daemon_pid`, retrieval/storage modes, and the last commit sequence. The local `memoree daemon status|stop|restart` wrappers use that result; stop/restart are restricted to the default private Unix endpoint so the CLI cannot signal a Compose- or supervisor-owned process by mistake.

`verify` checks schema state, FTS revision membership, and the integrity of referenced and external CAS blobs. Inspect its structured issues before trusting or backing up a damaged store. Full semantic comparison of indexed text is not part of the current verifier; the FTS projection remains derivable from authoritative revisions, but no public reindex command is exposed yet.
