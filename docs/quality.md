...
The mandatory baseline is structured lookup plus SQLite FTS5/trigram fusion. The optional dense projection has been admitted only as candidate recall because it improves candidate-pool coverage while leaving qualification unchanged. The optional cross-encoder remains claim-ordering-only, opt-in, and guarded; default-on promotion requires a positive cluster-robust top-3 uplift lower bound, at least 200 shadow queries with inference-only p95 at or below 500 ms, breaker trip rate below 1%, and no safety/facet regression. Artifact reranking requires its own measured uplift. External Meilisearch, vector-only retrieval, and generic hybrid RAG remain comparison configurations, not default dependencies.
