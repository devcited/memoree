...
- One-call retrieval preserves recall presence exactly, never broadens scope, returns at most 12 KiB/nine references, admits automatic recovery bytes only from exact evidence attached to candidate claims, labels every recovered byte unqualified, and keeps raw artifact/semantic-window bait out of the packet.
- Query intent/script profiles never change qualification, scope, lifecycle, citations, model authority, or recall presence. Explicit current/post-migration intent may suppress unqualified historical candidate hydration and route the caller to current source.
- Project indexing reads only bounded Git-visible text, excludes configured/sensitive/generated paths, binds citations to full working-tree hashes, refuses stale fetches, serializes rebuilds, and keeps the previous valid projection when an incremental change budget is exceeded. Structural mapping pins native grammars, returns at most eight leads/12 KiB, verifies every included current-source span, preserves duplicate targets as ambiguous, degrades parser failures to FTS per file, and leaves `not_ready` non-writing. Incremental and clean rebuild projections must remain equivalent. Continuous watching is explicit foreground opt-in.
...
The repository's `memoree-eval` binary implements the deterministic correctness/regression subset against the versioned corpora, including realistic v2 paraphrase, typo, long-document, scope, abstention, temporal, and conflict slices; see [Retrieval evaluation](evaluation.md). The mandatory one-call recovery gate is at least 90% recovery, at most 10% false abstention, zero false answers, scope/citation violations, semantic-bait fetches, and case-forbidden fetches, at most 12 KiB per packet, and tested-host p95 at or below two seconds. Recommended operational gates are warm recall p95 at or below 500 ms and the full pinned suite at or below ten minutes. Model-in-the-loop task success remains a separate benchmark.
...
`memoree profile retrieve` is privacy-safe operational instrumentation: output has a closed numeric/count allowlist and records neither the query nor retrieved content. It does not measure answer quality.

Continuous project metrics remain disabled until a project explicitly opts in. Their separate disposable database has an exact tested column allowlist and records minute timestamps, operation/outcome categories, counts, sizes, durations, and booleans only. It has no query, content, citation, path, prompt, model-name, free-text task, note, or raw-error channel. Recording is best-effort and cannot change the measured operation. `metrics doctor` must pass integrity, schema, categorical, private-permission, outside-project, retention, and size checks before an export is trusted.

Passive metrics are observational and cannot support a token-savings claim. The explicit experiment workflow generates opaque pairs, randomizes and enforces arm order, accepts immutable numeric/ordinal observations only, and summarizes complete pairs with medians, direction counts, order balance, and an exact sign test. Even then, fewer than 30 pairs require raw-pair inspection, one declared primary metric, consistent provider token accounting, and explicit caveats for learning, order, drift, optional stopping, and human grading.

The controlled 0.5.0 audit did not demonstrate token savings or better downstream quality at equal completeness. The 0.6 experimental project map also failed its downstream gate: its fresh complex held-out batch lost blind quality in 8/10 cases, and narrow tasks used 36–55% more processed input with no correctness gain. Memoree must not claim savings or downstream improvement from architectural intuition, retrieval-only scores, whole-corpus baselines, or graph-derived ground truth. A general token claim requires pre-registered real development-task A/B cases using the same model and task, no worse completeness or false-answer safety in every routed stratum, and at least 10% fewer processed tokens. Report latency, index cost, and model cost separately. Until that gate passes, the supported benefit is narrower: durable off-repository history with exact provenance and safe continuity.
