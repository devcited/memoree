...
mkdir -p "$package_dir" "$server_root/releases/download/v0.3.0" \
...
archive="$server_root/releases/download/v0.3.0/memoree-$target.tar.gz"
...
"$install_dir/memoree" --version | grep -F '0.3.0' >/dev/null
HOME="$user_root" MEMOREE_HOME="$memoree_home" \
  "$install_dir/memoree" doctor | grep -F '"binary_version":"0.3.0"' >/dev/null
(
  cd "$project_dir"
  HOME="$user_root" MEMOREE_HOME="$memoree_home" \
